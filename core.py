...
from csv_result import CSVTotalResultManager
...
async def main():
	...
	csv_results_manager = CSVTotalResultManager(start_time_str, test_suite_manager.get_test_suite_tags())
	...
	for mut_info in mut_infos:
		...
		for testsuite_tag in test_suite_manager.get_test_suite_tags():
			...
			test_suite_manager.run_test_suite(testsuite_tag, mut_info, start_time_str)  # run the test suite and save the result

			output = mutated_app_manager.stop()  # close mutated application
			save_app_output(mut_info.id, start_time_str, testsuite_tag, output)  # save output

		save_mut_info(mut_info, start_time_str)  # save json with all the info about this mutant
		csv_results_manager.append_result(mut_info)  # save all the info about this mutant in a CSV line
